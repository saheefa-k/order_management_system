defmodule OrderManagementSystem.Customers do
  @moduledoc """
  ...
  def subscribe_customers(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(OrderManagementSystem.PubSub, "user:#{key}:customers")
  end

  defp broadcast_customer(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(OrderManagementSystem.PubSub, "user:#{key}:customers", message)
  end
